...
NSString *const kGANAccountIdKeyP =          @"accountIdP";
...
+ (BOOL)launchWithOptions:(NSDictionary *)launchOptions
    ...
    NSString *accountId;
    if (easyTracker.isProduction) {
        accountId =
        [easyTracker getStringParameter:kGANAccountIdKeyP
                           defaultValue:nil
...
- (BOOL)parseParametersAndStartTracker:(NSError **)error {
  ...
  if (debugEnabled) {
    ...
    if (isProduction) {
        accountId =
        [self getStringParameter:kGANAccountIdKeyP
                           defaultValue:nil
                            description:@"account id"];
    } else {
        accountId =
  ...
  NSLog(@"Tracker started!");
    NSString *versionString = [[NSString alloc] initWithString:[[[NSBundle mainBundle] infoDictionary] objectForKey:@"CFBundleVersion"]];
    ...
    if(![[GANTracker sharedTracker] setCustomVariableAtIndex:2
                                                       ...
                                                       scope:kGANVisitorScope
                                                   withError:&error1]){
        NSLog(@"OS Version Error Occured visitor");
    }
